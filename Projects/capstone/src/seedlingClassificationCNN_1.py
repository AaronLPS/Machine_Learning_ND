...
# Put the image file name and its category to one pandas dataframe
trainData = []
...
# The following models was tested one by one.
# The first one is the Benchmark model. The last one is the final model with the best performance
...
# model = Sequential()
# model.add(Conv2D(16, (3, 3), input_shape=(DIM, DIM, 3)))   # define convolutional layer: 16 filters; kernel-size = (3,3);
# model.add(BatchNormalization(axis=3))                      # add BN layer before the activation function
# model.add(Activation('relu'))                              # define activation function = ReLU
...
# model.add(BatchNormalization(axis=3))
# model.add(Activation('relu'))
# model.add(GlobalMaxPooling2D())                            # add GMPooling at the last of convolutional volume
#
# model.add(Dense(64, activation='relu'))                    # define FC: 64 neurons; activation function=ReLU
# model.add(Dropout(0.5))
# model.add(Dense(32, activation='relu'))
# model.add(Dropout(0.5))
# model.add(Dense(12, activation='softmax'))                 # define the output of the last FC to be 12 dimensions, indicating 12 categories
# model.summary()
# model.compile(loss='categorical_crossentropy',             # define the loss, optimizer and metrics
...
model.add(Conv2D(24, 3, 3, border_mode="same", input_shape=(DIM, DIM, 3)))              # define convolutional layer
model.add(BatchNormalization(axis=3))                                                   # add BN layer
model.add(Activation('relu'))
model.add(Conv2D(24, 3, 3, subsample=(2, 2), border_mode="same"))                       # use subsample to reduce the CNN volume dimension
model.add(BatchNormalization(axis=3))
model.add(Activation('relu'))
...
# model.add(Dense(50))
# model.add(Activation('relu'))
model.add(Conv2D(64, 3, 3, subsample=(2, 2), border_mode="same"))                       # reduce the dimension to (2,2,64)
model.add(BatchNormalization(axis=3))
model.add(Activation('relu'))
model.add(GlobalAveragePooling2D())                                                     # add GAPooling layer
model.add(Dense(12, activation='softmax'))
model.summary()
model.compile(loss='categorical_crossentropy',                                          # define loss, optimizer and metrics
               optimizer=Adam(lr=1e-4), metrics=['acc'])                                # Keras 2.0 only supports metrics = "accuracy"
...
# define the callback function to save the modle
...
# Save the model when training finished
#todo
